...
public class ClaimCommandExecutor extends BaseCommandExecutor<InfinitePlots> {
	...
	public void plotClaim(CommandSender sender, String label, String[] args){
		...
		if (!Permission.PLOT_BYPASS_CLAIM_LIMIT.has(player)){
			...
			if (Permission.PLOT_LIMIT_PREMIUM.has(player)) {
				max = plugin.config.getInt(Config.CLAIM_MAX_PREMIUM);
			} else if (Permission.PLOT_LIMIT_INVESTOR.has(player)) {
				max = plugin.config.getInt(Config.CLAIM_MAX_INVESTOR);
			} else if (Permission.PLOT_LIMIT_BUILDER.has(player)) {
				max = plugin.config.getInt(Config.CLAIM_MAX_BUILDER);
			} else if (Permission.PLOT_LIMIT_VIP.has(player)) {
				max = plugin.config.getInt(Config.CLAIM_MAX_VIP);
			} else if (Permission.PLOT_LIMIT_NORMAL.has(player)) {
				max = plugin.config.getInt(Config.CLAIM_MAX_NORMAL);
			}
